...
export class WorldGenerator {
  ...
  private riverCache: Map<string, number> = new Map(); // Cache for river noise values
  ...
  generateChunk(chunkX: number, chunkY: number, chunkSize: number = 10): TerrainPoint[][] {
    ...
    // Batch generate river data
    this.batchGenerateRivers(coordinates);
  ...
  generateTerrainLine(
    ...
    // Batch generate height, climate, and river data
    this.batchGenerateHeights(coordinates, chunkSize);
    this.batchGenerateClimate(coordinates);
    this.batchGenerateRivers(coordinates);
  ...
  generateTerrainPoint(
    ...
    // Generate height, climate, and river data for the single point
  ...
  private batchGenerateRivers(coordinates: Array<{ x: number, y: number, worldX: number, worldY: number }>): void {
    // Batch generate river noise values for all coordinates
    for (const coord of coordinates) {
      const key = `${coord.worldX},${coord.worldY}`;

      if (!this.riverCache.has(key)) {
        const riverValue = this.noiseGen.generateRiverNoise(coord.worldX, coord.worldY);
        this.riverCache.set(key, riverValue);
      }
    }
  }
  ...
  private generateTerrainPointFast(x: number, y: number): TerrainPoint {
    ...
    const riverNoise = this.riverCache.get(`${x},${y}`)!;
    ...
    // Check if this should be a river before assigning other terrain properties
    if (this.shouldBeRiver(x, y, normalizedHeight, riverNoise)) {
      this.setAsRiver(point);
    } else {
  ...
  private shouldBeRiver(x: number, y: number, normalizedHeight: number, riverNoise: number): boolean {
    // Rivers only exist between sea level and high mountains
    if (normalizedHeight < this.seaLevel || normalizedHeight > 0.8) {
      return false;
    }

    // Use multiple noise layers to create river networks
    const riverThreshold = this.calculateRiverThreshold(x, y, normalizedHeight);

    return riverNoise > riverThreshold;
  }

  private calculateRiverThreshold(x: number, y: number, normalizedHeight: number): number {
    // Base threshold - higher values mean fewer rivers
    let threshold = 0.6;

    // Rivers are more likely at moderate elevations
    const elevationFactor = 1 - Math.abs(normalizedHeight - 0.55) * 2;
    threshold -= elevationFactor * 0.15;

    // Add some regional variation to river density
    const regionalVariation = this.noiseGen.generateRegionalRiverDensity(x, y);
    threshold += regionalVariation * 0.1;

    return Math.max(0.4, Math.min(0.8, threshold));
  }

  private manageCacheSize(): void {
    // Prevent memory bloat by clearing oldest entries when cache gets too large
    const caches = [this.heightCache, this.temperatureCache, this.precipitationCache, this.riverCache];
  ...
  private assignTerrainProperties(point: TerrainPoint): void {
    // Handle water tiles (oceans only now, since rivers are handled separately)
    if (point.w) {
