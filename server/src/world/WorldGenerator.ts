...
export class WorldGenerator {
  ...
  private postProcessChunk(terrain: TerrainPoint[][], chunkSize: number): void {
    ...
    // New noise-based river generation
    for (let y = 0; y < chunkSize; y++) {
      for (let x = 0; x < chunkSize; x++) {
        const point = terrain[y][x];
        ...
        if (point.w || point.nH < 0.3 || point.nH > 0.8) continue; // Skip oceans, out-of-range heights, existing water

        const riverVal = this.noiseGen.generateRiverValue(worldX, worldY);
        const widthVal = this.noiseGen.generateRiverWidth(worldX, worldY);

        // Dynamic threshold: base narrow, wider in some areas for lake-like; prefer lower elevations
        let thresh = 0.1 + widthVal * 0.3; // Tune: 0.1-0.4 range for narrow to wide
        const elevationFactor = 1 - (point.nH - 0.3) / 0.5; // 1 at 0.3, 0 at 0.8
        thresh *= elevationFactor; // Fewer rivers at higher elevations
